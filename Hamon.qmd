...
summary_by_group <- final_grades_df |>
  group_by(group) |>
  summarise(
    mean_final = mean(final_grade, na.rm = TRUE),
    median_final = median(final_grade, na.rm = TRUE),
    sd_final = sd(final_grade, na.rm = TRUE),
    n = n(),
    .groups = "drop"
  )
...
ggplot(final_grades_df, aes(x = factor(group), y = final_grade)) +
